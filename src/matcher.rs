...
mod tests {
    ...
    fn test1() {
        ...
        expected.insert(vec![Command(InsertString(Infinite, conv("12")))], s01.clone());
        expected.insert(vec![Command(InsertString(Infinite, conv("DE")))], s2.clone());

        expected.clear();
        let cmd_truncate3 = Command(Truncate(Val(3)));
        let res_truncate3 = worker_logic(vec![cmd_truncate3.clone()], &wordlist, &clears, 1);
        expected.insert(vec![cmd_truncate3.clone(), Command(InsertString(Infinite, conv("12")))], s01.clone());
        expected.insert(vec![cmd_truncate3, Command(InsertString(Infinite, conv("DE")))], s2.clone());
